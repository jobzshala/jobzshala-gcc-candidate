...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  const items: NavItem[] = [
    { key: "profile", label: t("dashboard.nav.profile"), href: "/dashboard/profile", icon: UserIcon },
    { key: "overview", label: t("dashboard.nav.myJourney"), href: "/dashboard", icon: CompassIcon },
    { key: "matches", label: t("dashboard.nav.matches"), href: "/dashboard/matches", icon: TargetIcon },
  ...
  return (
    <nav className="sidebar">
      ...
      {/* TODO(backend): no real support WhatsApp number is wired anywhere in
          this app yet (confirmed — login's WhatsApp button is a stubbed
          social-login action, not a support contact link). Disabled until
          a real number/deep link exists. */}
      <div className="help-card">
        <h4>Need Help?</h4>
        <p>Talk to your recruiter — we are here to help you 24/7</p>
        <button type="button" className="wa-btn" disabled title="Coming soon">
